...
commandStanfordEvents.prototype.helpResponse = function (response_url, output) {
  ...
  var text =
    ...
    '    */sevents featured*' + '\n' +
    '    */sevents*' + '\n' +
    'You can get a list of featured events for a specific month by entering (ex. for July):' + '\n' +
    '    */sevents month 7*' + '\n' +
...
commandStanfordEvents.prototype.postEventsResponse = function (events, replyMessageHeader, response_url) {
  var handleObject = this;
...
commandStanfordEvents.prototype.findEvents = function(get_url, replyMessageHeader, response_url) {
  ...
  request({
    ...
    handleObject.getEvents(xml, function(events) {
      ...
      handleObject.postEventsResponse(events, replyMessageHeader, response_url);
    });
...
commandStanfordEvents.prototype.handle = function (req, res, cb) {
  ...
  if (parts[0].trim() == 'help') {
    ...
    handleObject.helpResponse(response_url, '');
    return;
  }
  ...
  if (parts[0].trim() == 'month') {
    ...
    get_url = 'http://events.stanford.edu/xml/' + currentYear + '/' + monthName + '/eventlist.xml';
    handleObject.findEvents(get_url, 'Featured Events for ' + monthName + ': ', response_url);
    return;
  }

  // http://events.stanford.edu/xml/2016/June/22/eventlist.xml
