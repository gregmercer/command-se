...
commandStanfordEvents.prototype.getEvents = function (xml, cb) {
  ...
  var test = 'made as early as 4500�BCE';
  console.log(test);
  test = test.replace(/\xef/g, ' ');
  test = test.replace(/\xbf/g, ' ');
  test = test.replace(/\xbd/g, ' ');
  console.log(test);
  ...
  parseString(xml, function (err, result) {
    ...
    for (var index = 0; index < result.EventList.Event.length; index++) {
      ...
      var eventTime = event.beginTime + ' ' + event.endTime;
      var eventLocation = event.locationText[0];
      var eventUrl = event.url ? event.url[0] : '';

      //console.log('before eventDescription = ' + eventDescription);
      //eventDescription = eventDescription.replace('&nbsp;', ' ');
      eventDescription = eventDescription.replace(/\xef/g, ' ');
      eventDescription = eventDescription.replace(/\xbf/g, ' ');
      eventDescription = eventDescription.replace(/\xbd/g, ' ');
      //console.log('after eventDescription = ' + eventDescription);
