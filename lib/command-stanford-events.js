...
commandStanfordEvents.prototype.getEvents = function (xml, cb) {
  ...
  parseString(xml, function (err, result) {
    ...
    for (var index = 0; index < result.EventList.Event.length; index++) {
      ...
      var eventDate = '';
      var today = new Date();
      var metaDate = new Date(event.metaDate);
      if (today.getTime() > metaDate.getTime()) {
        eventDate = dateFormat(today, "dddd, mmmm dS, yyyy");;
      }
      else {
        // assumes metaDate and beginDate are same
        eventDate = event.beginDay + ', ' + event.beginDate;
      }
      ...
      var eventImageUrl = '';
      console.log(event.Media.imageUrl[0]);
      if (event.Media && event.Media.imageUrl) {
        eventImageUrl = event.Media.imageUrl[0];
      }
...
commandStanfordEvents.prototype.findFeatured = function(response_url) {
  var handleObject = this;

  var get_url = 'http://events.stanford.edu/xml/eventlist.xml';
  ...
  request({
    ...
    handleObject.getEvents(xml, function(events) {
      ...
      for (var index = 0; index < events.length; index++) {
        ...
        console.log('imageUrl = ' + events[index].eventImageUrl);
      ...
      var output = {
        response_type: "ephemeral",
        text: 'Featured Events:' + ' (' + events.length + ' results found)',
...
commandStanfordEvents.prototype.handle = function (req, res, cb) {
  ...
  if (parts[0].trim() == 'featured') {
    handleObject.findFeatured(response_url);
    return;
  }
