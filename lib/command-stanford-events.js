...
commandStanfordEvents.prototype.helpResponse = function (responseUrl, output) {
  ...
  var text =
    ...
    '    */sevents month aug*' + '\n' +
    'You can get a list of week\'s events by entering:' + '\n' +
    '    */sevents week*' + '\n' +
...
commandStanfordEvents.prototype.removeDuplicateEvents = function (events) {
  var handleObject = this;
  ...
  for (var prop in events) {
    var eventID = events[prop].eventID;
    if (!eventIDsList[eventID]) {
      eventIDsList[eventID] = events[prop];
      eventsOut[eventsOut.length] = events[prop];
    }
  }
...
commandStanfordEvents.prototype.postEventsResponse = function (events, replyMessageHeader, responseUrl) {
  ...
  events = handleObject.removeDuplicateEvents(events);
...
commandStanfordEvents.prototype.loopEventsByUrl = function (getUrls, index, responseUrl) {
  var handleObject = this;

  //console.log('in loopEventsByUrl getUrls.length: ' + getUrls.length);
  //console.log('in loopEventsByUrl index: ' + index);
  ...
  if (index < getUrls.length) {
    handleObject.getEventsByUrl(getUrls[index], function(err) {
      if (err) {
        ...
        handleObject.loopEventsByUrl(getUrls, index+1, responseUrl);
      }
    })
  }
  else {
    // found all the events for all the getUrls
    //console.log('end of loopEventsByUrl');
...
commandStanfordEvents.prototype.getEventsByUrl = function (getUrl, cb) {
  ...
  request({
    ...
    handleObject.getEvents(xml, function(events) {
      ...
      console.log(events);
      for (var index = 0; index < events.length; index++) {
        handleObject.allEvents[handleObject.allEvents.length] = events[index];
      }
      cb(null);
    });
...
commandStanfordEvents.prototype.handle = function (req, res, cb) {
  ...
  if (parts[0].trim() == 'week') {
    ...
    for (var index = 0; index < 7; index++) {
      ...
      var monthName = handleObject.monthLookupByNumber(currentMonthNumber);
    ...
    handleObject.loopEventsByUrl(getUrls, 0, responseUrl);
